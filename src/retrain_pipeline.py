...
def train_and_register_model():
    ...
    with mlflow.start_run() as run:
        ...
        if acc >= 0.90:
            ...
            print(
                f"Model successfully logged and registered under URI: {model_info.model_uri}"
            )
        else:
            print(
                "Candidate model failed accuracy threshold (0.90). Promotion skipped."
            )
...
def run_pipeline():
    ...
    if drift_detected:
        ...
        print(
            "No significant drift detected. Model operating within normal bounds."
        )
